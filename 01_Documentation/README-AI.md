...
| 2026-09-14 export fix | In the exported SVG the "planned" thin overlay bar was drawn at a hardcoded `y+10`/height 8, copied from the live view's 28px row height; the export draws rows at `rowH=24`, so the thin bar sat 2px below the row's middle line while the thick bar and baseline ghost (both scaled off `rowH`) stayed centered. | Thin overlay's y now `y+(rowH-8)/2`, centering it on the row like the other two. |
| 2026-09-14 audit | Doc drift: version pill section still described the `'Version 1'` label with a hover card showing publish date, author and email. | Pill relabelled `'Y26'`, then removed from the UI entirely per follow-up request; hover card and its state/handlers removed — pill is now a static label. Doc corrected to match. |
